...
import { ArrowUpRight, Clock, X, ChevronLeft, Tag, Loader2 } from 'lucide-react'
import type { AiNewsArticle } from '@/lib/notion'
...
function richText(items: { plain_text: string; annotations?: { bold?: boolean; italic?: boolean; code?: boolean } }[]) {
  return items.map((t, i) => {
    ...
    if (t.annotations?.bold) return <strong key={i} className="text-white/90">{t.plain_text}</strong>
    if (t.annotations?.italic) return <em key={i} className="italic">{t.plain_text}</em>
...
function BlockRenderer({ block }: { block: NotionBlock }) {
  ...
  switch (type) {
    case 'paragraph':
      ...
      return <p className="text-[14px] text-white/65 leading-relaxed mb-3">{richText(data.rich_text)}</p>
    case 'heading_1':
      return <h2 className="text-xl font-bold text-white/90 mt-7 mb-3">{richText(data.rich_text ?? [])}</h2>
    case 'heading_2':
      return <h3 className="text-lg font-semibold text-white/85 mt-6 mb-2">{richText(data.rich_text ?? [])}</h3>
    case 'heading_3':
      return <h4 className="text-base font-semibold text-white/80 mt-5 mb-2">{richText(data.rich_text ?? [])}</h4>
    case 'bulleted_list_item':
      return <li className="text-[14px] text-white/65 leading-relaxed mb-1.5 ml-4 list-disc">{richText(data.rich_text ?? [])}</li>
    case 'numbered_list_item':
      return <li className="text-[14px] text-white/65 leading-relaxed mb-1.5 ml-4 list-decimal">{richText(data.rich_text ?? [])}</li>
    case 'code':
      return (
        <pre className="my-4 p-4 rounded-xl bg-white/[0.05] border border-white/[0.08] overflow-x-auto">
          <code className="text-[12px] font-mono text-emerald-300/90 leading-relaxed">
            {(data.rich_text ?? []).map(t => t.plain_text).join('')}
          </code>
        </pre>
      )
    case 'quote':
      return <blockquote className="my-4 pl-4 border-l-2 border-white/20 text-[14px] text-white/50 italic leading-relaxed">{richText(data.rich_text ?? [])}</blockquote>
    case 'divider':
...
const categoryAccent: Record<string, string> = {
  'Model':      'text-violet-400 border-violet-400/30 bg-violet-400/10',
  'Araç':       'text-sky-400   border-sky-400/30   bg-sky-400/10',
  'Araştırma':  'text-amber-400 border-amber-400/30 bg-amber-400/10',
...
function Badge({ label, accent }: { label: string; accent: string }) {
  return (
    <span className={`text-[10px] font-medium tracking-widest uppercase px-2 py-0.5 rounded-full border ${accent}`}>
      {label}
    </span>
  )
}
...
/* ── PostRow ─────────────────────────────────────────────────────────── */
function PostRow({ article, isActive, onClick, hideExcerpt }: {
  article: AiNewsArticle; isActive: boolean; onClick: () => void; hideExcerpt: boolean
}) {
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5 mb-1 flex-wrap">
          {article.category && <Badge label={article.category} accent={accent} />}
        </div>
        ...
        {!hideExcerpt && article.summary && (
          <p className="text-xs text-white/40 leading-relaxed mb-1.5 line-clamp-2">{article.summary}</p>
        )}
        <div className="flex items-center gap-2 text-[10px] font-mono text-white/45">
          {article.date && <span>{new Date(article.date).toLocaleDateString('tr-TR', { day: 'numeric', month: 'short', year: 'numeric' })}</span>}
          {article.date && <span className="text-white/20">·</span>}
          <Clock size={9} />
          <span>3 dk</span>
        </div>
      </div>
...
/* ── Detail Panel ────────────────────────────────────────────────────── */
function DetailPanel({ article, onClose, mobile, blocks, blocksLoading }: {
  article: AiNewsArticle; onClose: () => void; mobile: boolean
  blocks: NotionBlock[]; blocksLoading: boolean
}) {
  const accent = getAccent(article.category)
  return (
    <motion.div
    ...
    >
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="px-6 md:px-10 pt-6 pb-16">
          ...
          <h2 className="text-xl md:text-2xl font-bold text-white leading-snug mb-3 tracking-tight">
            {article.title}
          </h2>

          <div className="flex items-center gap-3 text-[11px] font-mono text-white/25 mb-7 pb-6 border-b border-white/[0.07]">
            {article.date && <span>{new Date(article.date).toLocaleDateString('tr-TR', { day: 'numeric', month: 'long', year: 'numeric' })}</span>}
          ...
          {article.summary && (
            <p className="text-[14px] text-white/50 leading-relaxed border-l-2 border-white/10 pl-4 mb-7 italic">
              {article.summary}
            </p>
          )}
...
/* ── Main Component ──────────────────────────────────────────────────── */
export default function AiNewsClient({ articles }: { articles: AiNewsArticle[] }) {
  const [selected, setSelected] = useState<AiNewsArticle | null>(null)
  const [blocks, setBlocks] = useState<NotionBlock[]>([])
  const [blocksLoading, setBlocksLoading] = useState(false)
  const [activeCategory, setActiveCategory] = useState('Tümü')
  const [isMobile, setIsMobile] = useState(false)

  const categories = ['Tümü', ...Array.from(new Set(articles.map(a => a.category).filter(Boolean)))]
    ...
    <div className="flex h-full w-full overflow-hidden">
      ...
      >
        ...
        <div className="relative z-10 px-5 md:px-10 pt-20 md:pt-28 pb-16 max-w-2xl">
                ...
                <button
                  ...
                  className={`text-[11px] font-medium tracking-widest uppercase px-4 py-1.5 rounded-full border transition-all duration-300 ${
                    activeCategory === cat
                      ...
                      : 'text-white/45 border-white/15 hover:text-white/75 hover:border-white/30'
                  }`}
                >
                  {cat}
                </button>
          ...
          {/* Articles */}
          {articles.length === 0 ? (
            <p className="text-sm text-white/30 font-mono">Notion'da henüz yayınlanmış yazı yok.</p>
          ) : (
      ...
      <AnimatePresence>
        {selected && (
          isMobile
            ? <DetailPanel key="mobile" article={selected} onClose={() => setSelected(null)} mobile blocks={blocks} blocksLoading={blocksLoading} />
            : <DetailPanel key="desktop" article={selected} onClose={() => setSelected(null)} mobile={false} blocks={blocks} blocksLoading={blocksLoading} />
        )}
      </AnimatePresence>
    </div>
  )
